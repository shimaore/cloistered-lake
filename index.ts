...
export class LakeAsyncIterator<T> implements AsyncIterable<T>, AsyncIterator<T> {
  ...
  /** A LakeAsyncIterator is an AsyncIterator
   *
  ...
  /** A LakeAsyncIterator is an AsyncIterable
   * It can be turned back into a stream.
   */
   ...
   *
   * Consumes this stream; returns a Promise.
   *
   ...
   *
   * The Promise will reject if the stream or the reducer function fail for any reason.
   */
 ...
 *
 * Takes iterators, async iterators, iterables or async iterables, and returns an asyncIterator
 */
const mergeArray = async function*(streams: I<any>[]) : AsyncGenerator<[any,number],void,unknown> {
  ...
  let srcs = Array.from(sources.values());
  // While we still have any sources, race on the sources we have left
  while (sources.size) {
    ...
    if (winner.result.done) {
      ...
      // No, grab the value to yield for this round, and queue up the next
...
export const merge = function(...streams: I<any>[]) {
 ...
 *
 * From two LakeAsyncIterator instances or two iterators, returns a boolean Promise indicating
...
/** .combine(streamA,streamB)
 *
 * Combines a tuple of streams into a stream of tuples.
 */
// FIXME study how we can properly propagate types for more than two arguments
// (things like `Promise.all()`, `values.some()` don't seem powerful enough)
export function combine<T1,T2>(A:I<T1>, B:I<T2>) : LakeAsyncIterator<[T1,T2]>
{
  const generator = async function*() : AsyncGenerator<[T1,T2]> {
    const lakeA = from(A)
    const lakeB = from(B)
    let done = false
    while (!done) {
